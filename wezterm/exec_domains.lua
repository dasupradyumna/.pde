...
function docker.wrap_exec(name)
  return function(cmd)
    local auto_starter = wezterm.config_dir .. '/auto-start-docker.sh'
    local ok, _, error = wezterm.run_child_process { 'bash', auto_starter, name }
    if not ok then
      wezterm.log_error('exec_domains:: Failed to start docker container\n', error)
      return
    end

    cmd.args = cmd.args or { '/bin/sh' }
    local wrapped = { 'docker', 'exec', '-it', name }
    for _, arg in ipairs(cmd.args) do
      table.insert(wrapped, arg)
    end
    DEBUG('exec_domains:: Wrapped command:', wrapped)
...
function docker.exec_domains()
  ...
  for _, name in ipairs(docker.container_list()) do
    table.insert(exec_domains, wezterm.exec_domain('docker:' .. name, docker.wrap_exec(name)))
  end
